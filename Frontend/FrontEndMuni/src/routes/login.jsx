...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    setLoading(true);
    try {
      await login(userrut, password);
      navigate('/menu');
    } catch (error) {
      setErrors({ general: error.message }); // Show the actual error message
    } finally {
      setLoading(false);
    }
  };
              ...
              <form>
                <div className="mb-4">
                  ...
                  <input
                    ...
                    disabled={loading}
                  />
                  {errors.userrut && <p className="text-red-500 text-sm mt-1">{errors.userrut}</p>}
                </div>
                <div className="mb-6">
                  <label
                    className="block text-gray-700 text-sm font-bold mb-2"
                ...
                <button
                  ...
                  onClick = {handleLogin}
                  disabled={loading}
                  className={`w-full relative py-2.5 px-5 rounded-lg font-medium group overflow-hidden ${styles.gradientBorder} ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
